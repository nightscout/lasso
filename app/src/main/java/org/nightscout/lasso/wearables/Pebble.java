...
public class Pebble {
    ...
    private int lastUploaderBattery;
    private int lastReceiverBattery;
    ...
    public void sendDownload(GlucoseReading reading, TrendArrow trend, long recordTime, int uploaderBattery, int receiverBattery, Context cntx) {
        sendDownload(reading, trend, recordTime, cntx, uploaderBattery, receiverBattery, false);
    }

    public void sendDownload(GlucoseReading reading, TrendArrow trend, long recordTime, Context cntx, int uploaderBattery, int receiverBattery, boolean resend) {
        ...
        lastTrend = trend;
        lastDelta = delta;
        lastUploaderBattery = uploaderBattery;
        lastReceiverBattery = receiverBattery;
        ...
        PebbleDictionary dictionary = buildDictionary(trend, bgStr, (int) (recordTime / 1000),
                ...
                String.valueOf(uploaderBattery), pwdName);
    ...
    public void resendDownload(Context cntx) {
        if (currentReading != null) {
            sendDownload(lastReading, lastTrend, lastRecordTime, cntx, lastUploaderBattery, lastReceiverBattery, true);
        }
    }
